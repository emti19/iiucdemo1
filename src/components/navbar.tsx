...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="w-full bg-white shadow-sm relative z-50" style={{ backgroundColor: "var(--primaryGreen)" }}>
      {/* Top bar with quick links */}
      <div
        className="hidden md:block text-white text-sm px-0 py-1 w-full container mx-auto flex justify-between items-center"
      >
          ...
          <div className="flex flex-col gap-2">
            {[
              ...
              <div key={menu.label} className="flex flex-col">
                <span className="font-semibold text-gray-700">{menu.label}</span>
                {menu.items.map((item) => (
                  <a key={item} href="#" className="pl-4 py-1 text-gray-600 hover:bg-gray-100 rounded">{item}</a>
                ))}
              </div>
            ))}
            <button className="mt-2 px-6 py-2 rounded-full border border-green-700 text-[#005C25] font-semibold hover:bg-green-50 transition">Contact us</button>
            <button className="mt-2 px-6 py-2 rounded-full bg-[#005C25] text-white font-semibold hover:bg-green-800 transition">Log in</button>
          </div>
        </div>
      )}
    </nav>
